...
Every non-pet window uses `WindowDecorations="BorderOnly"` with `AemeathTitleBar`; `WindowsWindowFrame` suppresses the DWM border while preserving edge resizing. Controls require accessible names and visible focus. Use `DialogService` for modal flows; any dialog that already exposes `取消` hides the title-bar close control, while independent windows opt in explicitly. Settings navigation uses `SettingsPageId` and explicitly synchronizes selection with `SettingsContentHost.Content`; never restore `SelectedIndex` or `SelectedItem.Tag` bindings. Provider, Computer Control, and MCP dirty state compares normalized snapshots, and failed saves must not navigate. Feedback uses `AemeathToastHost` with 2.5-second success and 4.5-second error durations. Built-in Skill runtime persona stops at `<!-- persona-end -->`; research notes and authorization/source metadata must stay after the marker, while user Skills without it remain compatible. For OpenAI-compatible SSE, map only blank `finish_reason` values to JSON null and preserve every `delta`/tool-call fragment; never synthesize streaming IDs, names, or arguments. Empty visible responses render as transient actionable failures and never enter session or memory persistence. Thumbnail decoding must use delete-sharing source streams so background previews cannot block window shutdown or temporary-file cleanup.

Chat `SplitView` owns its outer margin: use a transparent pane, a 240 px card, and a 12 px gap. `ChatScrollViewer` and `MessagesPanel` stretch horizontally; constrain dynamic message rows from viewport layout events and never bind row `Width` to `MessagesPanel.Bounds`. Initial history pins to the latest message while Extent, Viewport, or content size is changing; a non-programmatic scroll more than 96 px away must cancel even a queued pin immediately so late layout cannot pull the user back. Voice input is click-driven (`开始录音` -> `结束录音` -> `正在识别…`) and auto-sends recognized text; pulse motion uses only transform/opacity and stops under Reduce Motion or minimized windows. Global scrollbars keep a 12 px hit target and 6 px thumb; the chat variant keeps that hit target but uses a 3 px visual track and 5 px thumb. Provider wide panes and card lists stretch vertically; narrow lists stay capped. Pet menu headers use `pet-menu-item`; actions such as `跟随鼠标` remain top-level.
